...
/*- set entry_offset = 0x00000 if is_64_bit else 0x8000 -*/
...
#warning Using standard Linux entry point, please consider setting kernel_entry_addr explicitly.
    .entry_addr =
        /*? vm_address_config.get('ram_base') ?*/ +/*? '0x%x'%entry_offset ?*/,
/*- endif -*/
...
#warning You are using the deprecated linux_address_config structure. Please use the vm_address_config structure instead
    ...
    .ram =
        {
            .phys_base =
                /*? linux_address_config.get('linux_ram_paddr_base') ?*/,
    ...
    .entry_addr = /*? linux_address_config.get('linux_ram_base') ?*/
    +/*? '0x%x'%entry_offset ?*/,
...
#warning                                                                       \
    "You are using the deprecated linux_image_config structure. The provide_initrd, generate_dtb, provide_dtb, \
...
#if defined(CONFIG_PLAT_EXYNOS5) || defined(CONFIG_PLAT_QEMU_ARM_VIRT) ||      \
    defined(CONFIG_PLAT_TX2)
    .map_one_to_one = 1,
#else
    .map_one_to_one = 0,
#endif
