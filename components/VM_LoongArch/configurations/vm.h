...
#define __CALL8(f, a, b, c, d, e, g, h, i)                                     \
...
#define __CALL_NARGS_FROM(...)                                                 \
  __CALL_NARGS_X(__VA_ARGS__, 8, 7, 6, 5, 4, 3, 2, 1, )
#define __CALL_DISP_FROM(f, b, ...)                                            \
  __CALL_CONCAT(b, __CALL_NARGS_FROM(__VA_ARGS__))(f, __VA_ARGS__)
...
#define VM_COMPONENT_DEF(num) component VM vm##num;

#define VM_COMPONENT_CONNECTIONS_DEF(num)                                      \
  connection seL4RPCDataport fs##num(from vm##num.fs, to fserv.fs_ctrl);       \
  connection seL4GlobalAsynch notify_ready_vm##num(                            \
      from vm##num.notification_ready_connector,                               \
      to vm##num.notification_ready);

#define VM_GENERAL_COMPOSITION_DEF() component FileServer fserv;

#define VM_COMPOSITION_DEF(num)                                                \
  VM_COMPONENT_DEF(num)                                                        \
  VM_COMPONENT_CONNECTIONS_DEF(num)

#define VM_GENERAL_CONFIGURATION_DEF() fserv.heap_size = 0x30000;
...
#define VM_VIRTUAL_SERIAL_COMPONENTS_DEF()                                     \
  ...
  connection seL4TimeServer serialserver_timer(from serial.timeout,            \
                                               to time_server.the_timer);

#define PER_VM_VIRTUAL_SERIAL_CONNECTIONS_DEF(num)                             \
  connection seL4SerialServer serial_vm##num(from vm##num.batch,               \
                                             to serial.processed_batch);       \
  connection seL4SerialServer serial_input_vm##num(                            \
      from vm##num.serial_getchar, to serial.getchar);
