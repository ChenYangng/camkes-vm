...
#include <loongarch_vm/gen_config.h>
...
seL4_CPtr WEAK camkes_dtb_get_nth_untyped(int n, size_t *size_bits,
                                          uintptr_t *paddr);
seL4_Error WEAK camkes_dtb_get_irq_cap(int irq, seL4_CNode cnode,
...
static void *vm_map_paddr_with_page_size(vm_io_cookie_t *io_mapper,
                                         uintptr_t paddr, size_t size,
                                         int page_size_bits, int cached) {
  ...
  for (unsigned int i = 0; i < num_pages; i++) {
    ...
    error = vka_utspace_alloc_at(
        vka, &path, kobject_get_type(KOBJECT_FRAME, page_size_bits),
        page_size_bits, start + (i * page_size), &cookies[i]);

    if (error) {
      cookies[i] = -1;
      error = simple_get_frame_cap(simple, (void *)start + (i * page_size),
                                   page_size_bits, &path);
      if (error) {
  ...
  void *vaddr = vspace_map_pages(vspace, frames, NULL, seL4_AllRights,
                                 num_pages, page_size_bits, cached);
  if (vaddr) {
    return vaddr + offset;
  }
error:
  for (unsigned int i = 0; i < num_pages; i++) {
    ...
    if (cookies[i] != -1) {
      vka_utspace_free(vka, kobject_get_type(KOBJECT_FRAME, page_size_bits),
                       page_size_bits, cookies[i]);
    }
    vka_cspace_free(vka, frames[i]);
  }
  return NULL;
...
/* Force the _dataport_frames  section to be created even if no modules are
 * defined. */
static USED SECTION("_dataport_frames") struct {
} dummy_dataport_frame;
  ...
  for (dataport_frame_t *frame = __start__dataport_frames;
    ...
    if (frame->paddr == paddr) {
      if (frame->size == size) {
        ...
        ZF_LOGF("ERROR: found mapping for %p, wrong size %zu, expected %zu",
                (void *)paddr, frame->size, size);
      }
    }
  }
  return NULL;
...
static void *vm_map_paddr(void *cookie, uintptr_t paddr, size_t size,
  ...
  for (int i = frame_size_index; i >= 0; i--) {
    void *result = vm_map_paddr_with_page_size(io_mapper, paddr, size,
                                               sel4_page_sizes[i], cached);
    if (result) {
      return result;
    }
  }
...
static int vm_new_io_mapper(simple_t simple, vspace_t vspace, vka_t vka,
  ...
  *cookie = (vm_io_cookie_t){.vspace = vspace, .simple = simple, .vka = vka};
  *io_mapper = (ps_io_mapper_t){.cookie = cookie,
                                .io_map_fn = vm_map_paddr,
                                .io_unmap_fn = vm_unmap_vaddr};
  return 0;
}

static seL4_Error vm_simple_get_irq(void *data, int irq, seL4_CNode cnode,
                                    seL4_Word index, uint8_t depth) {
  seL4_Error res;
...
/* First try and allocate TCBs from the CAmkES static pool before retyping from
 * Untypeds */
static vka_utspace_alloc_maybe_device_fn utspace_alloc_copy;
static vka_utspace_alloc_at_fn utspace_alloc_at_copy;

static int
camkes_vm_utspace_alloc_maybe_device_fn(void *data, const cspacepath_t *dest,
...
static int camkes_vm_utspace_alloc_at(void *data, const cspacepath_t *dest,
                                      seL4_Word type, seL4_Word size_bits,
                                      uintptr_t paddr, seL4_Word *res) {
  if (type == seL4_LOONGARCH_16K_Page) {
    for (dataport_frame_t *frame = __start__dataport_frames;
         frame < __stop__dataport_frames; frame++) {
      if (frame->paddr == paddr) {
        if (frame->size == BIT(size_bits)) {
          ...
          ZF_LOGF("ERROR: found mapping for %p, wrong size %zu, expected %zu",
                  (void *)paddr, frame->size, BIT(size_bits));
        }
      }
    }
  }
...
static int vmm_init(const vm_config_t *vm_config) {
  ...
  allocman = bootstrap_use_current_1level(
      simple_get_cnode(simple), simple_get_cnode_size_bits(simple),
      simple_last_valid_cap(simple) + 1 + num_extra_frame_caps,
      BIT(simple_get_cnode_size_bits(simple)), get_allocator_mempool_size(),
      get_allocator_mempool());
  assert(allocman);

  allocman_make_vka(vka, allocman);

  /* Overwrite alloc function with a custom wrapper for statically allocated
   * TCBs. */
  ...
  if (camkes_dtb_untyped_count) {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      printf("dtb untyped paddr: %" PRIu64 "\n", paddr);
      printf("dtb untyped cap: %" PRIu64 "\n", cap);
  ...
  err = sel4utils_bootstrap_vspace(
      vspace, &_alloc_data, simple_get_init_cap(simple, seL4_CapInitThreadPD),
      vka, NULL, NULL, existing_frames);
  assert(!err);

  /* Initialise device support */
  err = vm_new_io_mapper(*simple, *vspace, *vka, &_io_ops.io_mapper);
  assert(!err);
  ...
  //                              simple, simple_get_cnode(simple),
  //                              fault_ep_obj.cptr, IRQ_MESSAGE_LABEL, 256,
  //                              &_io_ops.malloc_ops);
  // assert(_irq_server);
  ...
  //     /* Create new IRQ server threads and have them allocate notifications
  //     for us */ thread_id_t t_id = irq_server_thread_new(_irq_server,
  //     seL4_CapNull,
  //                                              0, -1);
  //     assert(t_id >= 0);
  // }
...
//         &device); error = seL4_CNode_Revoke(root, ut, 32); assert(error ==
//         seL4_NoError);
//     }
//     /* delete anything from any slots that should be empty */
//     for (int i = simple_last_valid_cap(&simple) + 1; i <
//     BIT(simple_get_cnode_size_bits(&simple)); i++) {
...
//     morecore_top = (uintptr_t)
//     &morecore_area[CONFIG_LIB_SEL4_MUSLC_SYS_MORECORE_BYTES];
// }
...
//     /* If the acknowledge function pointer is NULL, this means that the
//     actual
...
// static void irq_handler(void *data, ps_irq_acknowledge_fn_t acknowledge_fn,
// void *ack_data)
// {
//     /* We don't actually acknowledge the IRQ yet, this is done later when we
//     update the VGIC's state. */ assert(data); irq_token_t token = data;
 ...
 */
static USED SECTION("_vmm_module") struct {
} dummy_module;
...
//     }
//
//     int err = vm_register_irq(vcpu, irq.irq.number, &do_irq_server_ack,
//     token); if (err == -1) {
//         return -1;
//     }
//
...
//     token->vm = vcpu->vm;
//
//     irq_id_t irq_id = irq_server_register_irq(irq_server, irq, handler,
//     token); if (irq_id < 0) {
//         return -1;
//     }
//
...
//     /* If explicitly requested, the CAmkES DTB can be ignored and one
//     provided
//      * by the file server can be used instead.
//      */
//     if ((NULL != vm_config->files.dtb_base) && ('\0' !=
//     vm_config->files.dtb_base[0])) {
...
//         } else {
//             size_t dtb_len = read(dtb_fd, gen_dtb_base_buf,
//             sizeof(gen_dtb_base_buf)); close(dtb_fd); if (dtb_len <= 0) {
...
//
//     fdtgen_context_t *context = fdtgen_new_context(gen_dtb_buf,
//     sizeof(gen_dtb_buf)); if (context == NULL) {
//         ZF_LOGE("Couldn't create fdtgen context");
//         return -1;
//     }
//
//     /* If VM has "plat_keep_devices" set, use it! Else, just use the default
//     */ int num_keep_devices = 0; char **keep_devices = NULL; if
//     (camkes_dtb_get_plat_keep_devices) {
//         keep_devices = camkes_dtb_get_plat_keep_devices(&num_keep_devices);
//     }
//     if (num_keep_devices) {
//         fdtgen_keep_nodes(context, (const char **)keep_devices,
//         num_keep_devices);
//     } else {
//         fdtgen_keep_nodes(context, plat_keep_devices,
//         ARRAY_SIZE(plat_keep_devices));
//     }
//
//     /* If VM has "plat_keep_devices and subtree" set, use it! Else, just use
//     the default */ int num_keep_devices_and_subtree = 0; char
//     **keep_devices_and_subtree; if
//     (camkes_dtb_get_plat_keep_devices_and_subtree) {
//         keep_devices_and_subtree =
//         camkes_dtb_get_plat_keep_devices_and_subtree(&num_keep_devices_and_subtree);
//     }
//     if (num_keep_devices_and_subtree) {
//         for (int i = 0; i < num_keep_devices_and_subtree; i++) {
//             fdtgen_keep_node_subtree(context, fdt_ori,
//             keep_devices_and_subtree[i]);
//         }
//     } else {
//         for (int i = 0; i < ARRAY_SIZE(plat_keep_device_and_subtree); i++) {
//             fdtgen_keep_node_subtree(context, fdt_ori,
//             plat_keep_device_and_subtree[i]);
//         }
//     }
//
//     for (int i = 0; i < ARRAY_SIZE(plat_keep_device_and_subtree_and_disable);
//     i++) {
//         fdtgen_keep_node_subtree_disable(context, fdt_ori,
//         plat_keep_device_and_subtree_and_disable[i]);
//     }
//     fdtgen_keep_nodes_and_disable(context, plat_keep_device_and_disable,
//     ARRAY_SIZE(plat_keep_device_and_disable));
//
//     int num_paths = 0;
//     char **paths = NULL;
...
static int alloc_vm_ram_cap(uintptr_t addr, vm_t *vm,
                            vm_frame_t *frame_result) {
  int err;
  cspacepath_t frame;
...
handle_on_demand_fault_callback(vm_t *vm, vm_vcpu_t *vcpu, uintptr_t fault_addr,
...
memory_fault_result_t unhandled_mem_fault_callback(vm_t *vm, vm_vcpu_t *vcpu,
  ...
  default:
    ...
    mapped =
        vm_map_reservation(vm, reservation, on_demand_iterator, (void *)vm);
    if (!mapped) {
...
static int main_continued(void) {
  ...
  /* install custom open/close/read implementations to redirect I/O from the VMM
   * to our file server */
  install_fileserver(FILE_SERVER_INTERFACE(fs));
  err = seL4_TCB_BindNotification(camkes_get_tls()->tcb_cap,
                                  notification_ready_notification());
  assert(!err);
  ...
  /* Create the VM */
  err = vm_init(&vm, &_vka, &_simple, _vspace, &_io_ops, _fault_endpoint,
                get_instance_name());
  assert(!err);
  ...
  vm.mem.map_one_to_one =
      vm_config.map_one_to_one; /* Map memory 1:1 if configured to do so */
...
int run(void) {
  ...
  if (&num_vcpus != NULL) {
    if (num_vcpus > CONFIG_MAX_NUM_NODES) {
      ZF_LOGE("Invalid 'num_vcpus' attribute setting: Exceeds maximum number "
              "of supported nodes. Capping value to CONFIG_MAX_NUM_NODES (%d)",
              CONFIG_MAX_NUM_NODES);
      NUM_VCPUS = CONFIG_MAX_NUM_NODES;
    } else if (num_vcpus <= 0) {
      ZF_LOGE("Invalid 'num_vcpus' attribute setting: Can't have 0 or negative "
              "amount of vcpus. Capping value to 1 vcpu (default value)");
    } else {
      NUM_VCPUS = num_vcpus;
    }
  }
